...
function injectCSS(){
  ...
  '.pfcur{display:flex; flex-wrap:wrap; gap:4px; background:var(--bg); border:1px solid var(--line); border-radius:999px; padding:3px; max-width:100%}'+
  '#v-portfolio{overflow-x:hidden}'+
  '#v-portfolio .scroll{max-width:100%}'+
  ...
  '.pfeye{padding:7px 10px; line-height:0; color:var(--dim)}'+
  '.pfeye:hover{color:var(--ink)}'+
  ...
  '.pfring{transform-box:fill-box; transform-origin:center; animation:pfPulse 1.5s ease-out 3 both}'+
  '.pfring2{animation-delay:.45s}'+
  '.pfringdot{transform-box:fill-box; transform-origin:center; animation:pfPop 4.6s ease-out 1 both}'+
  '@keyframes pfPulse{0%{transform:scale(.35); opacity:.85} 70%{opacity:.28} 100%{transform:scale(3.4); opacity:0}}'+
  '@keyframes pfPop{0%{transform:scale(0); opacity:0} 8%{transform:scale(1.35); opacity:1} 20%{transform:scale(1)} 88%{opacity:1} 100%{opacity:0}}'+
  '@media (prefers-reduced-motion:reduce){ .pfring{display:none} .pfringdot{animation:none} }'+
  ...
  '@media(max-width:720px){'+
    ...
    '#v-portfolio table:not(.pfclosed):not(.pfhist) thead th:nth-child(2),#v-portfolio table:not(.pfclosed):not(.pfhist) thead th:nth-child(3),'+
    '#v-portfolio table:not(.pfclosed):not(.pfhist) thead th:nth-child(4),#v-portfolio table:not(.pfclosed):not(.pfhist) thead th:nth-child(5),'+
    '#v-portfolio table:not(.pfclosed):not(.pfhist) tbody td:nth-child(2),#v-portfolio table:not(.pfclosed):not(.pfhist) tbody td:nth-child(3),'+
    '#v-portfolio table:not(.pfclosed):not(.pfhist) tbody td:nth-child(4),#v-portfolio table:not(.pfclosed):not(.pfhist) tbody td:nth-child(5){display:none}'+
    ...
    '#v-portfolio .pfcur button{padding:6px 10px; font-size:12px; border-radius:10px; flex:0 0 auto}'+
    '#pfSeriesSel{width:100%}'+
    '#pfChartView{margin-left:0!important; width:auto}'+
    '#v-portfolio .pfhead{flex-direction:column; align-items:stretch}'+
...
/* Eski "Gerçek Portföy" görünümü → "Uzun Vade Simülatör".
   Üst menüden kaldırıldı; Simülatör sekmesinin içindeki düğmeyle açılıyor. */
function hideRealPortfolio(){
  ...
  if(!document.getElementById('pfHideCss')){
    ...
    st.textContent='.tab[data-v="real"]{display:none!important}';
    document.head.appendChild(st);
  }
  /* Metinleri yeni ada göre güncelle */
  var s2r=$('simToReal'); if(s2r){ s2r.textContent='Tümünü uzun vadeye aktar'; s2r.style.display=''; }
  document.querySelectorAll('[data-real]').forEach(function(b){
    b.style.display=''; if(b.textContent.indexOf('Gerçeğe')>=0) b.textContent='→ Uzun vade';
  });
  /* Simülatör araç çubuğuna açma düğmesi ekle */
  if(!$('openLongSim')){
    var host=s2r?s2r.parentNode:null;
    if(host){
      var b=document.createElement('button');
      b.id='openLongSim'; b.className='ghost accent'; b.style.fontWeight='700';
      b.textContent='📈 Uzun Vade Simülatör';
      b.title='Uzun vadeli pozisyonlarını ayrı takip et';
      b.onclick=function(){ if(B&&B.goView) B.goView('real'); };
      host.insertBefore(b, host.firstChild);
    }
  }
  /* Uzun vade görünümüne geri dön düğmesi */
  var rv=$('v-real');
  if(rv && !$('longSimBack')){
    var bar=document.createElement('div');
    bar.style.cssText='display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:14px; flex-wrap:wrap';
    bar.innerHTML='<button class="back" id="longSimBack" style="margin-bottom:0">← Simülatör</button>'+
      '<h2 style="margin:0; font-size:20px">Uzun Vade Simülatör</h2>';
    rv.insertBefore(bar, rv.firstChild);
    var bk=bar.querySelector('#longSimBack');
    if(bk) bk.onclick=function(){ if(B&&B.goView) B.goView('sim'); };
  }
}
...
/* Gizli modda tutar ve adetler maskelenir; yüzdeler ve birim fiyatlar görünür kalır. */
var pfHide=false;
function pfMask(){ return '•••'; }
function pf$(v){ if(v==null) return '—'; return pfHide?pfMask():(psym()+fmt(v)); }
function pfMoney(sym,v,d){ if(v==null) return '—'; return pfHide?pfMask():(sym+fmt(v,d)); }
function pfQ(v,d){ if(v==null) return '—'; return pfHide?pfMask():fmt(v,d); }
/* Üstü çizili / normal göz ikonu (SVG — emoji değil) */
function pfEyeSVG(off){
  return '<svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round">'+
    '<path d="M1.5 12S5 5.5 12 5.5 22.5 12 22.5 12 19 18.5 12 18.5 1.5 12 1.5 12Z"/><circle cx="12" cy="12" r="3.2"/>'+
    (off?'<line x1="3" y1="21" x2="21" y2="3"/>':'')+'</svg>';
}
...
function pfSaveDay(){
  var v=pfSnapVals(); if(!v){ dlg('Kayıt yok','<p>Önce en az bir varlık ekle.</p>',[{label:'Tamam',primary:true}]); return; }
  var d=new Date().toISOString().slice(0,10), H=pf().hist, hc=H.ccy||'TRY';
  var old=null; (H.points||[]).forEach(function(p){ if(p.date===d) old=p; });
  var tt=pfTypeTotals(pcy());
  var labels={borsa:'Borsa', doviz:'Döviz', altin:'Altın', fon:'Fon', mevduat:'Mevduat', kripto:'Kripto', diger:'Diğer'};
  var lines='';
  if(tt){ for(var tk in PF_SERIES_MAP){ var k=PF_SERIES_MAP[tk]; if(tt[k]>0)
    lines+='<div class="prow" style="padding:5px 0"><span>'+labels[k]+'</span><b>'+pf$(tt[k])+'</b></div>'; } }
  dlg('Günü kaydet — '+pfDMY(d),
    '<div class="prow" style="padding:6px 0; border-bottom:1px solid var(--line)"><span><b>Toplam</b></span><b>'+pf$(tt?tt.total:0)+'</b></div>'+lines+
    '<div class="tpfield" style="margin-top:14px"><label>Güne ait not (ops.)</label>'+
    '<input class="fld" id="pfsdNote" style="width:100%" placeholder="örn. maaş girdi / altın aldım" value="'+((old&&old.note)?old.note.replace(/"/g,'&quot;'):'')+'">'+
    '<div class="tphelp">Notu olan günler grafikte kırmızı noktayla işaretlenir. Boş bırakabilirsin.</div></div>'+
    (old?'<div class="hint">Bugün için zaten bir kayıt var — üzerine yazılacak.</div>':''),
    [{label:'Kaydet',primary:true,fn:function(){ pfSaveDayDo(($('pfsdNote').value||'').trim()); }},{label:'Vazgeç'}]);
}
function pfSaveDayDo(note){
  var v=pfSnapVals(); if(!v) return;
  ...
  if(tt){
    ...
    var pt = old || {date:d};
    pt.value=tt.total;
    for(var mk in PF_SERIES_MAP) pt[PF_SERIES_MAP[mk]]=tt[PF_SERIES_MAP[mk]];
    if(note) pt.note=note; else delete pt.note;
    if(!old){ H.points.push(pt); H.points.sort(function(x,y){ return x.date<y.date?-1:1; }); }
  }
  pfPulseDate=d; pfChartView='chart';
  save(); pfRender();
}
...
function pfLineChart(pts, color, sym){
  ...
  [mn,(mn+mx)/2,mx].forEach(function(v){ var y=Y(v).toFixed(1);
    grid+='<line x1="'+PL+'" y1="'+y+'" x2="'+(W-PR)+'" y2="'+y+'" stroke="var(--line)" stroke-width="1"/>'+
      '<text x="'+(PL-6)+'" y="'+(Y(v)+3).toFixed(1)+'" text-anchor="end" font-size="10" fill="var(--faint)">'+(pfHide?'•••':((sym||'')+fmt(v,0)))+'</text>'; });
  var xlab='';
  ...
  var dots=pts.map(function(p,i){ if(!isNote(p)) return '';
    return '<circle cx="'+X(i).toFixed(1)+'" cy="'+Y(p.y).toFixed(1)+'" r="2.6" fill="#ef4444" stroke="var(--surface)" stroke-width="1" pointer-events="none"></circle>'; }).join('');
  /* her nokta için görünmez yakalama alanı — imleç nerede olursa olsun değer okunur */
  var band=(n>1)?((W-PL-PR)/(n-1)):(W-PL-PR);
  var hits=pts.map(function(p,i){
    var cx=X(i), x=Math.max(PL-band/2, cx-band/2);
    var tip=(pfDMY(p.date)+' · '+(pfHide?'•••':((sym||'')+fmt(p.y)))+(isNote(p)?(' · '+p.note):'')).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;');
    return '<rect class="pfhit" x="'+x.toFixed(1)+'" y="'+PT+'" width="'+Math.max(band,3).toFixed(1)+'" height="'+(H-PT-PB)+'" fill="transparent"'+
      ' data-cx="'+cx.toFixed(1)+'" data-cy="'+Y(p.y).toFixed(1)+'" data-tip="'+tip+'"'+(isNote(p)?' data-note="1"':'')+'></rect>';
  }).join('');
  /* yeni kaydedilen günü belli et: genişleyip sönen halka */
  var pulse='';
  if(pfPulseDate){ pts.forEach(function(p,i){ if(p.date!==pfPulseDate) return;
    var cx=X(i).toFixed(1), cy=Y(p.y).toFixed(1);
    pulse='<circle class="pfring" cx="'+cx+'" cy="'+cy+'" r="6" fill="none" stroke="'+color+'" stroke-width="2.5" pointer-events="none"/>'+
      '<circle class="pfring pfring2" cx="'+cx+'" cy="'+cy+'" r="6" fill="none" stroke="'+color+'" stroke-width="2" pointer-events="none"/>'+
      '<circle class="pfringdot" cx="'+cx+'" cy="'+cy+'" r="4" fill="'+color+'" stroke="var(--surface)" stroke-width="2" pointer-events="none"/>';
  }); }
  return '<div class="pfchart"><svg viewBox="0 0 '+W+' '+H+'" width="100%" preserveAspectRatio="xMidYMid meet">'+
    grid+'<path d="'+line+'" fill="none" stroke="'+color+'" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>'+dots+xlab+pulse+
    '<line class="pfguide" x1="0" y1="'+PT+'" x2="0" y2="'+(H-PB)+'" stroke="var(--faint)" stroke-width="1" stroke-dasharray="3 3" style="display:none" pointer-events="none"/>'+
    '<circle class="pfhov" r="4" fill="'+color+'" stroke="var(--surface)" stroke-width="2" style="display:none" pointer-events="none"/>'+
    hits+'</svg><div class="pftip" style="display:none"></div></div>';
}
function pfWireChart(box){ var wrap=box.querySelector('.pfchart'); if(!wrap) return;
  var tip=wrap.querySelector('.pftip'), guide=wrap.querySelector('.pfguide'), hov=wrap.querySelector('.pfhov');
  if(pfPulseDate && wrap.querySelector('.pfring')){
    try{ wrap.scrollIntoView({behavior:'smooth', block:'center'}); }catch(e){}
    setTimeout(function(){ pfPulseDate=null; }, 4800);
  }
  function place(ev){ var r=wrap.getBoundingClientRect();
    var x=ev.clientX-r.left, y=ev.clientY-r.top;
    tip.style.left=Math.min(Math.max(8,x+14), Math.max(8,r.width-tip.offsetWidth-8))+'px';
    tip.style.top=Math.max(4, y-tip.offsetHeight-10)+'px'; }
  wrap.querySelectorAll('.pfhit').forEach(function(h){
    h.addEventListener('mouseenter', function(){
      tip.textContent=h.getAttribute('data-tip'); tip.style.display='block';
      var cx=h.getAttribute('data-cx'), cy=h.getAttribute('data-cy');
      if(guide){ guide.setAttribute('x1',cx); guide.setAttribute('x2',cx); guide.style.display=''; }
      if(hov){ hov.setAttribute('cx',cx); hov.setAttribute('cy',cy);
        hov.setAttribute('fill', h.hasAttribute('data-note')?'#ef4444':hov.getAttribute('fill')); hov.style.display=''; }
    });
    h.addEventListener('mousemove', place);
    h.addEventListener('mouseleave', function(){ tip.style.display='none';
      if(guide) guide.style.display='none'; if(hov) hov.style.display='none'; });
  });
}
...
function pfRenderLock(box){
  box.innerHTML='<div class="card" style="max-width:380px; margin:44px auto; text-align:center">'+
    ...
    '<label style="display:flex; align-items:center; justify-content:center; gap:8px; margin:2px 0 12px; cursor:pointer; color:var(--dim); font-size:13px">'+
      '<input type="checkbox" id="pfHideChk"'+(pfHide?' checked':'')+' style="width:16px; height:16px; accent-color:var(--brand)">'+
      '<span>Tutarları gizli aç</span></label>'+
  ...
  var hc=$('pfHideChk'); if(hc) hc.onchange=function(){ pfHide=hc.checked; };
...
/* ---- Yıllıklandırılmış getiri (XIRR) ----
   Tarihli nakit akışları: alım = para çıkışı (−), satış = para girişi (+),
   bugün = mevcut toplam değer (+). Her akış kendi günkü kuruyla dc'ye çevrilir. */
function pfXIRR(dc){
  var flows=[], skipped=0, today=Math.floor(Date.now()/1000);
  Object.keys(pf().assets).forEach(function(id){ var a=pf().assets[id];
    if(a.valMode!=='qty' || !a.tx || !a.tx.length){
      if(a.valMode==='amount' && a.curValue!=null){
        var c0=(a.cost!=null&&!isNaN(a.cost))?a.cost:null;
        if(c0!=null && a.costAt){                                  /* tarihi varsa hesaba katılır */
          var ts0=Math.floor(a.costAt/1000);
          var amt0=convAtTime(c0, a.ccy||'TRY', dc, ts0); if(amt0==null) amt0=pfConv(c0, a.ccy||'TRY', dc);
          if(amt0!=null&&isFinite(amt0)){
            flows.push({t:ts0, cf:-amt0});
            var v0=aValueIn(a,dc); if(v0!=null&&v0>0) flows.push({t:today, cf:v0});
          } else skipped++;
        } else skipped++;
      }
      return; }
    a.tx.forEach(function(t){
      var ts=Math.floor(t.t/1000), pay=txPay(a,t);
      var amt=convAtTime(t.px*t.qty, pay, dc, ts); if(amt==null) amt=pfConv(t.px*t.qty, pay, dc);
      if(amt==null||!isFinite(amt)) return;
      flows.push({t:ts, cf:(t.side==='sell'?amt:-amt)});
    });
    var v=aValueIn(a,dc); if(v!=null&&v>0) flows.push({t:today, cf:v});
  });
  if(flows.length<2) return null;
  var pos=false, neg=false;
  flows.forEach(function(f){ if(f.cf>0) pos=true; if(f.cf<0) neg=true; });
  if(!pos||!neg) return null;
  var t0=flows.reduce(function(m,f){ return Math.min(m,f.t); }, flows[0].t);
  function npv(r){ var s=0;
    for(var i=0;i<flows.length;i++){ var yr=(flows[i].t-t0)/(365.2425*86400);
      s+=flows[i].cf/Math.pow(1+r, yr); }
    return s; }
  var lo=-0.9999, hi=10, flo=npv(lo), fhi=npv(hi);
  if(!isFinite(flo)||!isFinite(fhi)||flo*fhi>0) return null;
  for(var k=0;k<200;k++){ var mid=(lo+hi)/2, fm=npv(mid);
...
function aUnitTRYprev(a){                      /* 1 birimin dünkü TL karşılığı */
  ...
  if(a.curUnit!=null){                         /* adet bazlı fon/kripto vb., birim fiyatı elle */
    ...
    var b2=(pfSameDay(a.curAt) && a.prevUnit!=null)?a.prevUnit:a.curUnit;
    return b2*pc2;
  }
  return null;
}
function aDayIn(a, dc){
  ...
  if(a.valMode==='amount'){                    /* tutar bazlı: bugün elle güncellendiyse fark sayılır */
    if(a.curValue==null) return null;
    var cc=a.ccy||'TRY', pc=prevTRYper(cc); if(pc==null) return null;
    var prevRaw=(pfSameDay(a.curAt) && a.prevValue!=null)?a.prevValue:a.curValue;
    var nowV=frTRY(toTRY(a.curValue, cc), dc), prevV=(prevRaw*pc)/pd;
    if(nowV==null||!isFinite(prevV)||prevV===0) return null;
  ...
  var nv=frTRY(q*un, dc); if(nv==null) return null;
  var pv=(q*up)/pd;                             /* dünkü değer, DÜNKÜ kurla dc'ye çevrilir */
  if(!isFinite(pv)||pv===0) return null;
  return {now:nv, prev:pv, chg:nv-pv, pct:(nv/pv-1)*100};
}
...
/* Geçmiş verinin tablo görünümü — tüm kırılımlar tek ekranda */
function pfHistTable(){
  var H=pf().hist, hc=H.ccy||'TRY', d=pcy(), pts=(H.points||[]).slice();
  (pf().snaps||[]).forEach(function(s){ if(s[d]==null) return;
    var found=false; pts.forEach(function(p){ if(p.date===s.date) found=true; });
    if(!found) pts.push({date:s.date, _snap:s[d]}); });
  if(!pts.length) return '<div class="empty">Henüz geçmiş kaydı yok — "Günü kaydet" ya da geçmiş yükle.</div>';
  pts.sort(function(a,b){ return a.date<b.date?1:-1; });          /* en yeni üstte */
  var keys=[]; for(var tk in PF_SERIES_MAP) keys.push(PF_SERIES_MAP[tk]);
  var labels={borsa:'Borsa', doviz:'Döviz', altin:'Altın', fon:'Fon', mevduat:'Mevduat', kripto:'Kripto', diger:'Diğer'};
  var cols=keys.filter(pfHasField);
  var hasNote=pts.some(function(p){ var t=(p.note||'').trim().toUpperCase(); return !!t && t!=='YOK'; });
  var head='<tr><th class="l">Tarih</th><th>Toplam</th>'+
    cols.map(function(k){ return '<th>'+labels[k]+'</th>'; }).join('')+
    (hasNote?'<th class="l">Not</th>':'')+'</tr>';
  function cv(raw, date){ if(raw==null||isNaN(raw)) return null; var c=pfConvHist(raw, hc, d, date); return c.v; }
  var rows=pts.map(function(p){
    var tot=(p.value!=null)?cv(p.value,p.date):(p._snap!=null?p._snap:null);
    var note=(p.note||'').trim(); if(note.toUpperCase()==='YOK') note='';
    return '<tr><td class="l">'+pfDMY(p.date)+'</td><td><b>'+(tot==null?'—':pf$(tot))+'</b></td>'+
      cols.map(function(k){ var v=cv(p[k],p.date); return '<td>'+(v==null?'<span class="na">—</span>':pf$(v))+'</td>'; }).join('')+
      (hasNote?('<td class="l" style="white-space:normal; max-width:280px">'+(note?clean(note):'<span class="na">—</span>')+'</td>'):'')+'</tr>';
  }).join('');
  return '<div class="scroll" style="max-height:460px"><table class="pfhist"><thead>'+head+'</thead><tbody>'+rows+'</tbody></table></div>'+
    '<div class="hint" style="margin-top:6px">'+pts.length+' gün · değerler '+d+' cinsinden, her tarih kendi günkü kuruyla çevrildi.</div>';
}
...
function pfAssetRow(a){
  ...
  return '<tr data-open="'+a.id+'"><td><b>'+(a.name||clean(a.symbol||'—'))+'</b>'+(a.symbol?' <span class="nm">'+clean(a.symbol)+'</span>':'')+' <span class="pfbadge">'+pfTypeLabel(a.type)+'</span><div class="nm">'+(a.note?a.note.slice(0,28):aUnitName(a))+pfManualNote(a)+'</div></td>'+
    ...
    '<td><span class="nm" style="font-size:13px">'+pfMoney(psymCcy(nc),vn)+'</span></td>'+
...
function pfRenderList(box){ var P=pf(), t=pfTotals(), cur=pcy();
  ...
  var xr=pfXIRR(cur);
  var xTip='Yıllıklandırılmış getiri (XIRR): paranın ne zaman girip çıktığını hesaba katar. '+
    (xr?(Math.round(xr.days)+' günlük geçmiş · '+xr.n+' nakit akışı'+(xr.skipped?(' · '+xr.skipped+' tutar bazlı varlık tarihi olmadığı için hariç'):'')):'');
  var xCard = (xr && isFinite(xr.rate) && xr.days>30) ? ('<div><div class="k" title="'+xTip.replace(/"/g,'&quot;')+'">Yıllık getiri (XIRR)'+(xr.skipped?' *':'')+'</div><div class="v '+pnlCls(xr.rate)+'">'+
      (xr.rate>0?'+':'')+fmt(xr.rate,1)+'%</div></div>') : '';
  ...
  var viewBtns='<div class="pfcur" id="pfChartView" style="display:inline-flex; margin-left:auto">'+
    '<button data-cv="chart" class="'+(pfChartView==='chart'?'on':'')+'">Grafik</button>'+
    '<button data-cv="table" class="'+(pfChartView==='table'?'on':'')+'">Tablo</button></div>';
  ...
  if(pfChartView==='table'){ chart=pfHistTable(); selBtns=''; }
  ...
  box.innerHTML=''+
    ...
    '<div style="display:flex; align-items:center; gap:8px">'+
      '<button class="ghost pfeye" id="pfEyeBtn" title="'+(pfHide?'Tutarları göster':'Tutarları gizle')+'" aria-label="'+(pfHide?'Tutarları göster':'Tutarları gizle')+'">'+pfEyeSVG(pfHide)+'</button>'+
      '<button class="ghost" id="pfLockBtn" title="Kilitle ve panoya dön">🔒 Kilitle</button>'+
      '<div class="pfcur" id="pfCur">'+curBtns+'</div></div></div>'+
    stat+costWarn+
  ...
  pfWireChart(box);
  pfOdoApply(box);
  var cvb=$('pfChartView'); if(cvb) cvb.querySelectorAll('button').forEach(function(b){ b.onclick=function(){ pfChartView=b.dataset.cv; pfRender(); }; });
  var eb=$('pfEyeBtn'); if(eb) eb.onclick=function(){ pfHide=!pfHide; pfStatPrev={}; pfRender(); };
  ...
  if(isSell){                                   /* SAT: mevcut varlıklardan seç */
    ...
    box.innerHTML='<div class="log" style="max-height:280px; overflow:auto">'+list.map(function(a){
      ...
      return '<div class="e pfpick" data-id="'+a.id+'" style="cursor:pointer"><span><b>'+(a.name||clean(a.symbol||'—'))+'</b>'+
        ...
        '<span class="t">'+pfQ(q,q%1?4:0)+' '+aUnitName(a)+' · '+pf$(aValueDisp(a))+' ›</span></div>'; }).join('')+'</div>'+
...
function pfManualStock(sym, asset){
  ...
  dlg(isNew?('Manuel hisse ekle — '+clean(sym)):((asset.name||clean(asset.symbol||''))+' — manuel hisse'),
    ...
    (isNew?('<div class="tpfield"><label>Alış adedi</label><input class="fld" id="msQty" type="number" step="any" style="width:100%"></div>'+
      '<div class="tpfield"><label>Alış tarihi</label><input class="fld" id="msDate" type="date" value="'+new Date().toISOString().slice(0,10)+'" style="width:100%"></div>'+
    ...
    [{label:isNew?'AL':'Kaydet',primary:true,fn:function(){
      ...
      if(asset){ asset.market=mk; asset.ccy=ccy;
        if(!isNaN(cur) && cur!==asset.curUnit){ if(asset.curUnit!=null) asset.prevUnit=asset.curUnit; asset.curAt=Date.now(); }
...
function pfRenderAsset(box, a){
  ...
  function d$(v){ return pfMoney(dsym,v); }
  ...
  var metrics='<div class="metrics">'+
    '<div class="metric"><div class="k">Adet ('+unit+')</div><div class="v">'+pfQ(q,q%1?4:0)+'</div></div>'+
...
function pfTxDialog(asset, newSym, forceSide){
  ...
  dlg((asset?((asset.name||clean(asset.symbol||''))+' — '+(isSell?'SAT':'AL')):('AL — '+clean(newSym))),
    ...
    '<div class="tpfield"><label>Tarih</label><input class="fld" id="pftxDate" type="date" value="'+new Date().toISOString().slice(0,10)+'" style="width:100%"><div class="tphelp">Önce tarihi seç — o günün kapanış fiyatı aşağıya öneri olarak gelir.</div></div>'+
    '<div class="tpfield"><label>Fiyat ('+psymCcy(nc)+')</label><input class="fld" id="pftxPx" type="number" step="any" style="width:100%"'+((s&&s.price!=null&&!isSell)?(' value="'+s.price+'"'):'')+'><div class="tphelp">Öneriyi dilediğin gibi değiştirebilirsin.</div></div>'+
...
function pfBuyGold(asset, forceSide){
  ...
  dlg('Gram Altın — '+(isSell?'SAT':'AL'),
    ...
    '<div class="tpfield"><label>Tarih</label><input class="fld" id="pgDate" type="date" value="'+new Date().toISOString().slice(0,10)+'" style="width:100%"><div class="tphelp">Önce tarihi ve para birimini seç — gram fiyatı aşağıya öneri olarak gelir.</div></div>'+
    '<div class="tpfield"><label>Gram fiyatı ('+(isSell?'sattığın':'ödediğin')+' para biriminde)</label><input class="fld" id="pgPx" type="number" step="any" style="width:100%"><div class="tphelp">Öneri güncel gram fiyatından türetilir; geçmiş tarihlerde sen düzelt.</div></div>'+
...
function pfBuyFx(asset, forceSide){
  ...
  dlg('Döviz — '+(isSell?'SAT':'AL'),
    ...
    '<div class="tpfield"><label>Tarih</label><input class="fld" id="pxDate" type="date" value="'+new Date().toISOString().slice(0,10)+'" style="width:100%"><div class="tphelp">Önce tarihi ve para birimlerini seç — o günün kuru aşağıya öneri olarak gelir.</div></div>'+
    '<div class="tpfield"><label>Birim fiyat (1 birim döviz = kaç '+(isSell?'karşılık':'ödeme')+' parası)</label><input class="fld" id="pxPx" type="number" step="any" style="width:100%"><div class="tphelp">Öneriyi dilediğin gibi değiştirebilirsin.</div></div>'+
...
function pfAddOther(editId, preType){ var a=editId?pf().assets[editId]:null;
  ...
  var body='<div class="tpfield"><label>Ad</label><input class="fld" id="pfaName" style="width:100%" placeholder="örn. Vadeli TL / QNB Fonu"></div>'+
    ...
    '<div class="tpfield"><label>Alış / başlangıç tarihi</label><input class="fld" id="pfaDate" type="date" value="'+new Date().toISOString().slice(0,10)+'" style="width:100%"><div class="tphelp">Paranın ne zaman girdiği — yıllık getiri (XIRR) hesabı bunu kullanır.</div></div>'+
  ...
  if(a){ $('pfaName').value=a.name||''; $('pfaType').value=a.type; $('pfaCcy').value=a.ccy||'TRY'; $('pfaMode').value=a.valMode||'amount';
    ...
    var dEl=$('pfaDate');
    if(dEl){ var src=a.costAt || (a.tx&&a.tx.length?a.tx[0].t:null);
      if(src) dEl.value=new Date(src).toISOString().slice(0,10); } }
...
function pfSaveOther(editId){ var name=$('pfaName').value.trim(); if(!name) return false;
  ...
  if(!editId){ var ex=pfAssetForName(name,typeSel);
    if(ex){
      var exD=$('pfaDate')?$('pfaDate').value:'', exT=exD?new Date(exD+'T12:00:00').getTime():Date.now();
      if(ex.valMode==='qty'){ var uu=parseFloat($('pfaUnit').value); if(!isNaN(uu)) ex.curUnit=uu;
        ...
        if(!isNaN(sq)&&sq>0){ ex.tx=ex.tx||[]; ex.tx.push({t:exT,side:'buy',qty:sq,px:(isNaN(sa)?(ex.curUnit||0):sa),note:$('pfaNote').value||'ekleme'}); } }
      else { var vv=parseFloat($('pfaVal').value); if(!isNaN(vv)){ if(ex.curValue!=null && vv!==ex.curValue){ ex.prevValue=ex.curValue; ex.curAt=Date.now(); } ex.curValue=vv; }
  ...
  var pfaD=$('pfaDate')?$('pfaDate').value:'';
  var pfaT=pfaD?new Date(pfaD+'T12:00:00').getTime():Date.now();
  if(a.valMode==='amount'){ var vv2=parseFloat($('pfaVal').value); var nv2=isNaN(vv2)?null:vv2;
    if(editId && nv2!=null && a.curValue!=null && nv2!==a.curValue){ a.prevValue=a.curValue; a.curAt=Date.now(); }
    a.curValue=nv2; var cc2=parseFloat($('pfaCost').value); a.cost=isNaN(cc2)?null:cc2;
    if(pfaD) a.costAt=pfaT; }
  else { var uu2=parseFloat($('pfaUnit').value); var nu2=isNaN(uu2)?null:uu2;
    if(editId && nu2!=null && a.curUnit!=null && nu2!==a.curUnit){ a.prevUnit=a.curUnit; a.curAt=Date.now(); }
    a.curUnit=nu2;
    if(!editId){ var sq2=parseFloat($('pfaQty').value),sa2=parseFloat($('pfaAvg').value); if(!isNaN(sq2)&&sq2>0) a.tx=[{t:pfaT,side:'buy',qty:sq2,px:(isNaN(sa2)?(a.curUnit||0):sa2),note:'başlangıç'}]; } }
